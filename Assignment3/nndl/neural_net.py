...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        dscores /= N
        grads['W2'] = np.dot(h1.T, dscores).T + reg * W2 
        grads['b2'] = np.sum(dscores, axis=0)

        dh1 = np.dot(dscores, W2)
        drelu = (h1 > 0) * dh1
        grads['W1'] = np.dot(X.T, drelu).T + reg * W1
    ...
    def predict(self, X):
        ...
        relu = lambda x: np.maximum(0, x)
        h1 = relu(np.dot(X, self.params['W1'].T)+self.params['b1'])
        scores = np.dot(h1, self.params['W2'].T)+self.params['b2']
