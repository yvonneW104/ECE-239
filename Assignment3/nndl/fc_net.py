...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss = softmax + 0.5*self.reg*(np.sum(self.params['W2']**2)+np.sum(self.params['W1']**2))
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            previous_output, cache[i] = affine_relu_forward(previous_output, self.params['W%d'%(i+1)], self.params['b%d'%(i+1)])
        
        scores, cache[self.num_layers] = affine_forward(previous_output, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
